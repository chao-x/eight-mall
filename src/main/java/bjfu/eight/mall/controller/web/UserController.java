...
public class UserController {
    ...
    public HTTP updateUser(@RequestBody User user,HttpSession session) {
        ...
        user.setId(((User)((User) session.getAttribute("user"))).getId());
    ...
    public HTTP checkUserAsw(@RequestBody User user) {
        ...
        if(check.getAsw()==user.getAsw()&&user.getQuestion()==check.getQuestion()){
        ...
        switch (info.getType()) {
            case "account": {
                for (int i = 0; i < list.size(); i++) {
                    if (list.get(i).getAccount() == info.getInfo()) {
            ...
            case "email": {
                for (int i = 0; i < list.size(); i++) {
                    if (list.get(i).getEmail() == info.getInfo()) {
            ...
            case "phone": {
                for (int i = 0; i < list.size(); i++) {
                    if (list.get(i).getPhone() == info.getInfo()) {
